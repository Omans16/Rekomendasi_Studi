...
@push('scripts')
    <script src="{{ asset('js/admin/hasil-prediksi.js') }}" defer></script>
@endpush

@section('content')

<div class="admin-hasil-page">
    ...
    @isset($detail)
                    ...
                    <span class="result-meta-item">
                        ...
                        <span class="result-meta-val">
                            ...
                            @if(!empty($detail->jurusan_smk_lengkap) && $detail->jurusan_smk_lengkap !== $detail->jurusan_smk)
                                <span class="jurusan-fullname">
                                    {{ $detail->jurusan_smk_lengkap }}
                                </span>
                            @endif
                        </span>
                    </span>
                ...
                <div class="card card-summary">
                    ...
                    <div class="score-bar-wrap">
                        <div class="score-bar-label">
                            <span>Peluang Studi Lanjut — Random Forest</span>
                            <span>{{ $probRF }}%</span>
                        </div>
                        ...
                        <div class="score-note">
                            Angka ini menunjukkan seberapa besar sistem membaca kecenderungan siswa untuk melanjutkan kuliah berdasarkan data akademik.
                        </div>
                    </div>
                    ...
                    @if($rekUniv->isNotEmpty())
                        <div class="score-bar-wrap">
                            <div class="score-bar-label">
                                <span>Kemiripan dengan Alumni — KNN</span>
                                <span>{{ $avgSim }}%</span>
                            </div>
                        ...
                        <div class="formula-box">
                            Final Score = (0.7 × similarity_score) + (0.3 × frequency_score)
                        </div>
            ...
            <div>
                <div class="card card-recommendation">
                    ...
                    <div class="card-sub rekomendasi-sub">
                        Berdasarkan data yang dimasukkan, sistem menemukan kemiripan dengan data alumni jurusan
                        <strong>{{ $detail->jurusan_smk }}</strong> yang melanjutkan kuliah.
                    </div>
                    ...
                    @if($isKuliah && $kualitas)
                        <div class="quality-box">
                            ...
                            <div class="quality-desc">
                                Status ini menunjukkan apakah rekomendasi cukup relevan berdasarkan kemiripan profil siswa dengan data alumni.
                            </div>
                        </div>
                    @endif
                    ...
                    @else
                        <div class="empty-recommendation">
                            ...
                            <div class="empty-recommendation-desc">
                                Rekomendasi hanya tersedia untuk siswa yang teridentifikasi memiliki potensi studi lanjut.
                                Admin atau Guru BK tetap dapat menggunakan hasil ini sebagai bahan evaluasi awal.
                            </div>
                        </div>
                    @endif
                </div>
        ...
        <div class="result-action-grid">
            <div class="action-row result-action-bottom">
                <a href="{{ route('admin.hasil.prediksi') }}" class="btn btn-primary btn-sm action-link">
                    Riwayat
                </a>

                <a href="{{ route('admin.input.siswa') }}" class="btn btn-primary btn-sm action-link">
                    Prediksi Baru
                </a>
            </div>
    ...
    @isset($data)
        ...
        <form method="GET" action="{{ route('admin.hasil.prediksi') }}" class="filter-form">
            ...
            @if(request()->hasAny(['jurusan','status']))
                <a href="{{ route('admin.hasil.prediksi') }}" class="reset-link">
                    Reset
                </a>
            @endif
        </form>
        ...
        @if($data->count())
            ...
            <script type="application/json" id="adminHasilPrediksiRows">
                @json($allRowsData)
            </script>
        ...
        @else
            <div id="hasil-placeholder" class="card card-placeholder">
                ...
                <a href="{{ route('admin.input.siswa') }}" class="btn btn-primary btn-sm">
                    Input Data Siswa
                </a>
            </div>
        @endif
